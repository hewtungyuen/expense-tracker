...
const deleteConfirmation = async (ctx) => {
  ...
  const canDelete = await canDeleteExpense(
    telegramId,
    inOverseasMode,
    latestExpense
  );
  ...
  ctx.reply(
    `Delete expense: ${latestExpense.expenseDescription}?`,
  ...
  await api.patch(`/users/${telegramId}`, {
    currentState: state.DELETE_PREVIOUS_EXPENSE,
  });
};
...
const canDeleteExpense = async (telegramId, inOverseasMode, latestExpense) => {
  ...
  if (inOverseasMode && isOverseasExpense) {
    const currentTripName = await api
