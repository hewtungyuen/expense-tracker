...
const saveExpenseToDatabase = async (ctx) => {
  ...
  const description = await api
    .get(`/users/${telegramId}/expenseDescription`)
    .then((value) => value.data);
  const category = await api
    .get(`/users/${telegramId}/expenseCategory`)
    .then((value) => value.data);
  const expenseAmountSgd = await api
    .get(`/users/${telegramId}/expenseAmountSgd`)
    .then((value) => value.data);
  ...
  const overseasMode = await api
    .get(`/users/${telegramId}/overseasMode`)
    .then((value) => value.data);
  const expenseAmountOverseas = await api
    .get(`/users/${telegramId}/expenseAmountOverseas`)
    .then((value) => value.data);
  ...
  if (overseasMode) {
    const tripName = await api
      ...
      .then((value) => value.data);
    const exchangeRate = await api
      .get(`/users/${telegramId}/exchangeRate`)
      .then((value) => value.data);
    ...
    if (expenseAmountOverseas) {
      ...
      expense.expenseAmountSgd = (expenseAmountOverseas / exchangeRate).toFixed(
        2
      );
    }
  }
